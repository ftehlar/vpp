  ...
  for (i = 0; i < count; i += 16)
    {
      ...
      r.x4[2] = u8x64_xor3 (r.x4[2], aes_block_load_x4 (src, i), k[0].x4[2]);
      r.x4[3] = u8x64_xor3 (r.x4[3], aes_block_load_x4 (src, i), k[0].x4[3]);

      for (j = 1; j < rounds; j++)
	{
	  r.x4[0] = aes_enc_round_x4 (r.x4[0], k[j].x4[0]);
	  r.x4[1] = aes_enc_round_x4 (r.x4[1], k[j].x4[1]);
	  r.x4[2] = aes_enc_round_x4 (r.x4[2], k[j].x4[2]);
	  r.x4[3] = aes_enc_round_x4 (r.x4[3], k[j].x4[3]);
	}
      r.x4[0] = aes_enc_last_round_x4 (r.x4[0], k[j].x4[0]);
      r.x4[1] = aes_enc_last_round_x4 (r.x4[1], k[j].x4[1]);
      r.x4[2] = aes_enc_last_round_x4 (r.x4[2], k[j].x4[2]);
      r.x4[3] = aes_enc_last_round_x4 (r.x4[3], k[j].x4[3]);
